...
class scrapytest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.browser.get(url)
        ac = cls.browser.find_element_by_css_selector("[class='works-intro-view ui-btn-orange ui-radius3']")
    ...
    def test_scrapy_comic(self):
        ...
        while self.title <= self.end_of_episode:
            try:
                cheak = self.browser.find_element_by_xpath("//div[@class='mod_958wr mod_wbg mod_gbd mod_of']")
            ...
            try:
                right_title = int(re.search("第\d+话",self.browser.find_element_by_xpath("//span[@class='title-comicHeading']").text).group(0)[1:-1])
            except :
                print("无法获取当前正确的话数")
