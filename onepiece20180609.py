...
class Crawler(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            cls.browser = webdriver.Chrome(dir+'\chromedriver.exe', chrome_options=chrome_options)
            # chrome_options.add_argument('--headless')
            # chrome_options.add_argument('--disable-gpu')
        except:
            print("未发现chrome浏览器，正在尝试ie浏览器")
        if not cls.browser:
            try:
                if 'PROGRAMFILES(X86)' in os.environ:
                    cls.browser = webdriver.Ie(dir+'\IEDriverServer64.exe')
                else:
                    cls.browser = webdriver.Ie(dir + '\IEDriverServer32.exe')
                cls.browser.maximize_window()
            except:
                print("请检查浏览器设置或者driver是否正确放置")
                raise NoSuchElementException("请检查浏览器设置或者driver是否正确放置")
        ...
        cls.browser.get(url)
        ac = WebDriverWait(cls.browser, 10).until(
    ...
    def test_scrapy_comic(self):
        path_of_save = input("请输入爬取后的文件的储存位置\n")
        if not os.path.exists(path_of_save):
            print("非法路径")
            if os.path.exists("D:\\"):
                print("路径已自动设置为D:\\tem\\")
                path_of_save = 'D:\\tem\\'
        ...
        while self.title <= self.end_of_episode:
            try:
                cheak = WebDriverWait(self.browser, 2).until(expected_conditions.visibility_of_element_located(
            ...
            try:
                right_title = int(re.search("第\d+话", WebDriverWait(self.browser, 2).until(
                    expected_conditions.visibility_of_element_located(
                        (By.CLASS_NAME, "title-comicHeading"))).text).group(0)[1:-1])
            except:
                print("无法获取当前正确的话数")
                right_title = self.title
            ...
            if not right_title < self.title:
                ...
                path = path_of_save + str(right_title) + '\\'
                c = canvas.Canvas(path_of_save + str(right_title) + '.pdf', pagesize=(822, 1200))
