...
def crawling_comic(q, crawling_settings2):
    ...
    while settings['start_episode'] <= settings['end_of_episode']:
        ...
        try:
            ...
            print("已到最后一话，第%d话。" % settings['start_episode'])
            break
        settings['start_episode'] += 1

    q.put(settings['start_episode'])
    browser.quit()
...
if __name__ == '__main__':
    # 加载配置文件
    try:
        settings_file_1 = shelve.open("settings.db")
        crawling_settings = settings_file_1['key1']
    ...
    try:
        settings_file_1 = shelve.open("settings.db")
        settings_file_1['key1'] = crawling_settings
        settings_file_1.close()
    except:
        print("配置文件保存失败")
    print("共花费时间", end='')
